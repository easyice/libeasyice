...
static void ref_pic_list_reordering(Slice *currSlice,Bitstream *currStream);
static void pred_weight_table(Slice *currSlice,seq_parameter_set_rbsp_t *active_sps,pic_parameter_set_rbsp_t *active_pps,Bitstream *currStream);
#if (MVC_EXTENSION_ENABLE)
static void ref_pic_list_mvc_modification(Slice *currSlice,Bitstream *currStream);
#endif
 ...
 ************************************************************************
 */
int FirstPartOfSliceHeader(Slice *currSlice,Bitstream *currStream)
{
  ...
  currSlice->slice_type = (SliceType) tmp;
 ...
 ************************************************************************
 */
int RestOfSliceHeader(Slice *currSlice,seq_parameter_set_rbsp_t *active_sps,pic_parameter_set_rbsp_t *active_pps,Bitstream *currStream)
{
 /* VideoParameters *p_Vid = currSlice->p_Vid;
...
//  Bitstream *currStream = partition->bitstream;
  ...
  currSlice->num_ref_idx_active[LIST_0] = active_pps->num_ref_idx_l0_active_minus1 + 1;
  currSlice->num_ref_idx_active[LIST_1] = active_pps->num_ref_idx_l1_active_minus1 + 1;
  ...
  if (currSlice->svc_extension_flag == 0 || currSlice->svc_extension_flag == 1)
    ref_pic_list_mvc_modification(currSlice,currStream);
  else
    ref_pic_list_reordering(currSlice,currStream);
#else
  ...
  currSlice->weighted_pred_flag = (unsigned short) ((currSlice->slice_type == P_SLICE || currSlice->slice_type == SP_SLICE) 
    ...
    : (currSlice->slice_type == B_SLICE && active_pps->weighted_bipred_idc == 1));
  currSlice->weighted_bipred_idc = (unsigned short) (currSlice->slice_type == B_SLICE && active_pps->weighted_bipred_idc > 0);

  if ((active_pps->weighted_pred_flag&&(currSlice->slice_type==P_SLICE|| currSlice->slice_type == SP_SLICE))||
      (active_pps->weighted_bipred_idc==1 && (currSlice->slice_type==B_SLICE)))
  {
    pred_weight_table(currSlice,active_sps,active_pps,currStream);
  }

  if (currSlice->nal_reference_idc)
    dec_ref_pic_marking(currStream, currSlice);

  if (active_pps->entropy_coding_mode_flag && currSlice->slice_type!=I_SLICE && currSlice->slice_type!=SI_SLICE)
  {
  ...
  if(currSlice->slice_type==SP_SLICE || currSlice->slice_type == SI_SLICE)
  {
    ...
    currSlice->qs = 26 + active_pps->pic_init_qs_minus26 + val;    
    if ((currSlice->qs < 0) || (currSlice->qs > 51))
      error ("slice_qs_delta makes slice_qs_y out of range", 500);
  }

  if ( !HI_INTRA_ONLY_PROFILE || (HI_INTRA_ONLY_PROFILE && (intra_profile_deblocking == 1) ))
...
static void pred_weight_table(Slice *currSlice,seq_parameter_set_rbsp_t *active_sps,pic_parameter_set_rbsp_t *active_pps,Bitstream *currStream)
{
 /* VideoParameters *p_Vid = currSlice->p_Vid;
  ...
  DataPartition *partition = &(currSlice->partArr[dP_nr]);
  Bitstream *currStream = partition->bitstream;*/
  short bitdepth_luma       = (short) (active_sps->bit_depth_luma_minus8 + 8);
  short bitdepth_chroma   = (short) (active_sps->bit_depth_chroma_minus8 + 8);
  ...
  for (i=0; i<currSlice->num_ref_idx_active[LIST_0]; i++)
  {
    ...
    if (luma_weight_flag_l0)
    {
      ...
      currSlice->wp_offset[0][i][0] = currSlice->wp_offset[0][i][0]<<(bitdepth_luma - 8);
    }
    else
    {
    ...
    if (active_sps->chroma_format_idc != 0)
    {
      ...
      for (j=1; j<3; j++)
      {
        if (chroma_weight_flag_l0)
        {
          ...
          currSlice->wp_offset[0][i][j] = currSlice->wp_offset[0][i][j]<<(bitdepth_chroma-8);
        }
        else
        {
  ...
  if ((currSlice->slice_type == B_SLICE) && active_pps->weighted_bipred_idc == 1)
  {
    for (i=0; i<currSlice->num_ref_idx_active[LIST_1]; i++)
    {
      ...
      if (luma_weight_flag_l1)
      {
        ...
        currSlice->wp_offset[1][i][0] = currSlice->wp_offset[1][i][0]<<(bitdepth_luma-8);
      }
      else
      {
      ...
      if (active_sps->chroma_format_idc != 0)
      {
        ...
        for (j=1; j<3; j++)
        {
          if (chroma_weight_flag_l1)
          {
            ...
            currSlice->wp_offset[1][i][j] = currSlice->wp_offset[1][i][j]<<(bitdepth_chroma-8);
          }
          else
          {
